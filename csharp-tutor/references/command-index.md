# C# Tutor Skill Family

Use this index when the user asks what C# Tutor can do or which C# skill to select from the Codex picker.

## Core Skill

- `csharp-tutor` / `C# Tutor`: General C# tutoring, code review, refactoring, modernization, security, performance, testing, project inspection, and exercises.
...
- `csharp-collections` / `C# Collections`: Arrays, lists, dictionaries, sets, queues, stacks, `IEnumerable`, equality comparers, mutation, ordering, and choosing data structures.
- `csharp-architecture` / `C# Architecture`: Clean architecture, layered architecture, vertical slices, DTO boundaries, dependency direction, modularity, scalability, and testability.
- `csharp-concurrency` / `C# Concurrency`: Threads, locks, synchronization, concurrent collections, channels, `Parallel`, race conditions, shared state, cancellation, and CPU-bound parallel work.
...
@C# LINQ Explain IEnumerable and deferred execution.
@C# Errors Should I use exceptions or a Result type here?
@C# ASP.NET Review this minimal API endpoint for validation and authorization.
@C# EF Core Why is this query slow and how do I avoid N plus one queries?
@C# DI Help me choose lifetimes for these services.
@C# Generics Explain generic constraints with a small example.
@C# Collections Should this use a List, Dictionary, or HashSet?
@C# Architecture Review this app structure for maintainability.
@C# Concurrency Review this shared dictionary updated by multiple tasks.
